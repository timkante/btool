...
std::vector<BibElement> Parser::parseFile(boost::filesystem::ifstream &fsStream) {
    ...
    while (std::getline(fsStream, line)) {
        ...
        if (std::regex_search(line, group, expr)) {
            ...
            Field field = parseField(line, targetStructure);
            if (field.name != "") {
...
Field Parser::parseField(std::string field, std::optional<StyleProperties> targetStructure) {
    ...
    if (std::regex_search(field, group, fieldExpr)) {
        ...
        if (std::find(targetStructure->requiredFields.begin(), targetStructure->requiredFields.end(), fieldName) !=
            ...
            || std::find(targetStructure->optionalFields.begin(), targetStructure->optionalFields.end(), fieldName) !=
...
std::string Parser::replaceUmlaut(std::string value) {
    std::regex expr{"(\\{.*?\\})"};
    std::smatch group;
    std::regex_token_iterator<std::string::iterator> rend;
    std::regex_token_iterator<std::string::iterator> tokenIterator(value.begin(), value.end(), expr);
    if (std::regex_search(value, group, expr)) {
        //std::cout << group[1] << std::endl;
        std::cout << value << std::endl;

        while (tokenIterator != rend) {
            std::cout << *tokenIterator++ << "\t";
        }
